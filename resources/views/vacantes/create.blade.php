...
@section('content')
    <h1 class="mt-10 text-2xl text-center">Nueva Vacante</h1>

    <form action="{{ route('vacantes.store') }}" method="POST" class="mx-auto max-w-lg my-10">
        @csrf
        <div class="mb-5">
            <label for="titulo" class="block text-gray-700 text-sm mb-2">Título Vacante:</label>

            <input type="text" id="titulo" name="titulo" class="p-3 bg-gray-100 form-input rounded w-full" placeholder="Titulo de la vacante" value="{{ old('titulo') }}">
